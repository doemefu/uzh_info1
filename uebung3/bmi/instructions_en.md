...
Write a program that determines the right BMI catergory for a given height and weight. ~~The height in metres and weight in kg are provided in a `float` variables called `height` and `weight`, respectively. The BMI category is calculated by a function called `get_bmi_category` and your task is to complete the function.~~ Write a function that determines the right BMI category for a given height and weight. The function should take two parameters, the height in metres and weight in kg. It should return a different string depending on the function input parameters.
